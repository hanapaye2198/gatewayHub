...
new class extends Component {
    #[Layout('layouts.app', ['title' => 'GatewayHub Fees'])]
    ...
    #[Computed]
    public function totalPlatformFee(): float
    {
    ...
    #[Computed]
    public function platformFeeRows()
    {
    ...
    #[Computed]
    public function recentPlatformFeeWalletEntries()
    {
        $mid = $this->merchantId();
        if ($mid === null) {
            return collect();
...
<div class="flex h-full w-full flex-1 flex-col gap-6">
    <div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
        <h1 class="text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{{ __('GatewayHub Fees') }}</h1>
        <p class="mt-1 text-zinc-600 dark:text-zinc-400">{{ __('Track gross amount, GatewayHub platform fee, and net settlement per payment.') }}</p>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
            <p class="text-sm text-zinc-500 dark:text-zinc-400">{{ __('GatewayHub Fee Deducted') }}</p>
            <p class="mt-2 font-mono text-2xl font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">{{ number_format($this->totalPlatformFee, 2) }} PHP</p>
        </div>
        <div class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 [&_tbody_tr:hover]:bg-zinc-50 dark:[&_tbody_tr:hover]:bg-zinc-700/50 [&_tbody_tr]:border-b [&_tbody_tr]:border-zinc-200/60 [&_tbody_tr:last-child]:border-b-0 dark:[&_tbody_tr]:border-zinc-600/40">
        <h2 class="mb-3 text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ __('GatewayHub Fee Per Payment') }}</h2>
        <flux:table>
            <flux:table.columns>
            ...
            <flux:table.rows>
                ...
                @empty
                    <flux:table.row>
                        <flux:table.cell colspan="5" class="py-8 text-center text-zinc-500 dark:text-zinc-400">{{ __('No GatewayHub fee records yet.') }}</flux:table.cell>
                    </flux:table.row>
                @endforelse
            </flux:table.rows>
        </flux:table>
    ...
    <div class="overflow-hidden rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-800 [&_tbody_tr:hover]:bg-zinc-50 dark:[&_tbody_tr:hover]:bg-zinc-700/50 [&_tbody_tr]:border-b [&_tbody_tr]:border-zinc-200/60 [&_tbody_tr:last-child]:border-b-0 dark:[&_tbody_tr]:border-zinc-600/40">
        <h2 class="mb-3 text-sm font-semibold text-zinc-900 dark:text-zinc-100">{{ __('Recent GatewayHub Fee Wallet Entries') }}</h2>
        <flux:table>
            <flux:table.columns>
                <flux:table.cell variant="strong">{{ __('Reference') }}</flux:table.cell>
            ...
            <flux:table.rows>
                @forelse ($this->recentPlatformFeeWalletEntries as $entry)
                    <flux:table.row>
                ...
                @empty
                    <flux:table.row>
                        <flux:table.cell colspan="4" class="py-8 text-center text-zinc-500 dark:text-zinc-400">{{ __('No GatewayHub fee wallet entries yet.') }}</flux:table.cell>
                    </flux:table.row>
                @endforelse
            </flux:table.rows>
        </flux:table>
