...
@section('content')
    <div class="flex h-full w-full flex-1 flex-col gap-6 px-4 sm:px-6 lg:px-8">
        <div class="rounded-2xl border border-zinc-200 bg-white px-8 py-7 shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    ...
                    <h1 class="mt-4 text-2xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">{{ __('Users') }}</h1>
                    <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
                        {{ __('People who can sign in for this merchant. Each user belongs only to :merchant.', ['merchant' => $merchant->name]) }}
                    </p>
                </div>
                <flux:button variant="primary" icon="plus" :href="route('admin.merchants.users.create', $merchant)" wire:navigate>
                    {{ __('Create User') }}
                </flux:button>
            </div>
        ...
        <div class="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/60 dark:bg-zinc-900">
            @if ($users->isEmpty())
                <p class="px-6 py-10 text-sm text-zinc-500 dark:text-zinc-400">{{ __('No users yet for this merchant.') }}</p>
            @else
                        ...
                        <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-zinc-600 dark:text-zinc-300">{{ __('Merchant user') }}</td>
                                    <td class="px-6 py-4">
